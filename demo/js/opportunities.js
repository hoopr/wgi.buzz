...
var locationColors = { // Object of location fill and stroke colors
                      "ann arbor": ['#EDED6A', '#F4F4A6'],
                      boulder: ['#008EE5', '#66BBEF'],
                      honolulu: ['#936FEE', '#BEA9F5'],
                      portland: ['#FFA435', '#FFC886'],
                      "san francisco": ['#FC5561', '#FD99A0'],
                      tampa: ['#00B058', '#66D09B']
                     },
    divisionTextures = { // Object of textures for division backgrounds
                        "business development": "url(#dot)",
                        engineering: "url(#vertical-stripe)",
                        marketing: "url(#horizontal-stripe)",
                        "product management": "url(#crosshatch)",
                        sales: "url(#diagonal-stripe)"
                       },
    tooltipLabels = [ // Array of labels to build a tooltip
                     'NUMBER', 'LOCATION', 'DIVISION',
                     'PM', 'CLIENT', 'MARKETING COORD',
                     'FEES', 'PREREQS COMPLETE', 'STIPEND AMOUNT',
...
function init() {
  ...
  var requestURL = '/data/opportunities.json';
...
function filterData() {
  ...
  _.each(rawData, function(project) {
    ...
    for (var col in project) {
      ...
      if (['PROJECT NAME', 'LOCATION', 'DIVISION', 'PHASE 1 START', 'PHASE 1 END', 'PHASE 2 START', 'PHASE 2 END/PHASE 3 START', 'PHASE 3 END'].indexOf(col) === -1) {
    ...
    if (project['PHASE 1 START'] && project['PHASE 1 END'] && (filterValues.indexOf(project['DIVISION']) !== -1) && (filterValues.indexOf(project['LOCATION']) !== -1)) {
      phase1['id'] = projCount;
      phase1['START DATE'] = project['PHASE 1 START'];
      phase1['END DATE'] = project['PHASE 1 END'];
    ...
    if (project['PHASE 2 START'] && project['PHASE 2 END/PHASE 3 START'] && (filterValues.indexOf(project['DIVISION']) !== -1) && (filterValues.indexOf(project['LOCATION']) !== -1)) {
      phase2['id'] = projCount;
      phase2['START DATE'] = project['PHASE 2 START'];
      phase2['END DATE'] = project['PHASE 2 END/PHASE 3 START'];
    ...
    if (project['PHASE 2 END/PHASE 3 START'] && project['PHASE 3 END'] && (filterValues.indexOf(project['DIVISION']) !== -1) && filterValues.indexOf(project['LOCATION']) !== -1) {
      phase3['id'] = projCount;
      phase3['START DATE'] = project['PHASE 2 END/PHASE 3 START'];
      phase3['END DATE'] = project['PHASE 3 END'];
...
function drawSVG() {
  ...
  vizPhases.append('rect')
    .attr('height', function(d) {return hScale(createNumber(d['FEES']))}) // Height by fee amount
    ...
    .attr('stroke-width', 2)
    .attr('transform', function(d) {return 'translate(0, ' + (((h / projectLength) - hScale(createNumber(d['FEES']))) / 2) + ')'}) // Move to middle of row (row height - half its height)
    .attr('width', 0)
